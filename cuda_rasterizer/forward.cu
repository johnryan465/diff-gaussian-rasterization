	...
	float* __restrict__ out_depth)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// We are computing the expected depth of the ray termininating in
			// the current gaussian. This is approximated by the depth of the current
			// gaussian multiplied by the probability of the ray terminating in the gaussian
			// which is the opacity of the gaussian multiplied by the probability of the ray
			// reaching the gaussian which is the product of (1-alpha) for all previous gaussians.

			dep += collected_depth[j] * alpha * T;
